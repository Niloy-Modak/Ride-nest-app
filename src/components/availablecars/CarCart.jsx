...
const CarCart = ({ car, viewMode = 'grid' }) => {
    ...
    return (
        ...
        className={`bg-red-50 p-4 lg:p-8 rounded-2xl shadow-md transition-all duration-300 ${isList ? 'flex gap-4 items-start' : 'flex flex-col justify-between'}`}>
            {/* Car Image */}
            <div className={`${isList ? 'w-40 h-28 flex-shrink-0' : 'w-full h-[260px]'} overflow-hidden rounded-lg`}>
                <img
                    src={image}
                    alt={carName}
            ...
            <div className={`flex flex-col justify-between ${isList ? 'flex-1' : 'mt-4 space-y-4'}`}>
                <div className="flex justify-between">
                    <div>
                        <h1 className="text-xl md:text-2xl font-semibold">{carName}</h1>
                        <p className="flex items-center text-sm text-gray-700">
                            <FaLocationArrow className="mr-1" /> {location}
                        </p>
                    </div>
                    <div className="text-right">
                        <h2 className="text-xl md:text-2xl font-medium text-secondary flex items-center justify-end">
                ...
                <div className={`${isList ? 'mt-4 flex justify-between items-center ' : ''}`}>
                    <button className={`${isList ? 'p-1 cursor-pointer text-xs md:text-base px-4 rounded-full text-white text-shadow-2xs bg-green-500' : 'hidden'}`}>
                        {availability}
                    </button>
                    <Link to={`/car-details/${_id}`}>
                        <ButtonII ClsName={`${isList ? 'text-xs' : 'w-full'}`} label={`${isList ? 'Book Now' : 'Book Now'}`} />
                    </Link>
                </div>
            </div>
        </motion.div>
